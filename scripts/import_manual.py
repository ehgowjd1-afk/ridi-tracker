...
TABLES = {
    ...
    "1650-MONTHLY": [
        ...
        "꽃은 미끼야", "터치 유어 바디(Touch Your Body)", "한 명의 난초가 되기까지",
        "인투 더 섀도우(Into the shadow)", "룰 브레이커(Rule Breaker)", "이주", "연정이 꺾이다",
        "가시 돋친 순애", "첫사랑 따위 개나 줘", "페이드인(FADE IN)",
        # 21~43 (두 번째 이미지)
        "아 템포, 데이지", "언더커버 브이로그", "들개는 포식하는 꿈을 꾼다", "류안", "시절연애",
        ...
        "더티 어프로치", "스위트 스폿(Sweet Spot)", "제철 맞은 로맨스",
        # 44~60 (세 번째 이미지)
        "연하의 연하", "시대가 원하는 인재상", "파멸의 낙원", "피치핑크 #2791", "리얼 페이크 러브",
        "더티 에어", "크래시게이트(Crashgate)", "크레이지 페어", "교란종(攪亂種)", "노하우 다이렉트",
        "닳고 달은 아저씨", "야행(夜行)", "음란한 좋은 선배", "XOXO, 미스 미니", "목화다방",
        "여우 덫", "불량 식품에 길들여지면",
    ],
    "1700-WEEKLY": [
...
def norm(s):
    ...
    괄호 안 부제(예: "(Rule Breaker)")는 떼어 같은 작품끼리 묶는다.
    단, '개정판'은 본편과 완전히 다른 상품(ID)이라 반드시 구분해야 한다.
    (예: "하나의 더치" 3049080915  vs  "하나의 더치 (15세 개정판)" 3049080916)
    괄호를 통째로 떼면 둘이 같아져 본편이 두 번 박히므로, 개정판 표시만 남긴다.
    """
    s = s or ""
    is_rev = "개정판" in s
    base = re.sub(r"[^가-힣A-Za-z0-9]", "", re.sub(r"[\(\[（].*?[\)\]）]", "", s)).lower()
    return base + ("+개정판" if is_rev else "")
